...
|---|---|---|---|
| G-01 | **Transport identities derived from a public value.** Unless `node.json` sets `devnet.kem_seed_salt_hex` and `devnet.libp2p_seed_salt_hex`, the ML-KEM identity keypair and the libp2p identity (hence the PeerId) are derived from `node_id` alone, and `node_id` is public (status endpoint, logs, ceremony output). Anyone who knows it can recompute both long-term transport secrets without touching the disk. | **Closed at the source (2026-08-25).** `pqcd ceremony` generates a libp2p salt and a KEM salt per role, writes them into each role's `node.json` and derives the bootstrap PeerIds with them; the Ansible template takes `viper_libp2p_seed_salt_hex` / `viper_kem_seed_salt_hex` from the vault. | Remaining: the salts live in `node.json` (a ConfigMap in the chart, like the validator seeds it already carries) — moving them to a Secret is TASK-242's job. |
