...
fn headless_multiaddr(fullname: &str, role: &str, ns: &str) -> String {
    let peer_id = crate::p2p::deterministic_peer_id(&pod_name(fullname, role, 0), None);
...
fn pod_multiaddr(fullname: &str, role: &str, ordinal: u32, ns: &str) -> String {
    let pod = pod_name(fullname, role, ordinal);
    let peer_id = crate::p2p::deterministic_peer_id(&pod, None);
...
pub fn generate_ceremony_values(
    ...
    let validator_multiaddr = headless_multiaddr(&fullname, "validator", &cfg.namespace);
    let sentry_replicas: u32 = 2;
    let sentry_multiaddrs: Vec<String> = (0..sentry_replicas)
        .map(|i| pod_multiaddr(&fullname, "sentry", i, &cfg.namespace))
        .collect();
