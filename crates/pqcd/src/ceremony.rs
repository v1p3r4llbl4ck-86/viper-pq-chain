...
struct Libp2pSection {
    ...
    /// ADR-069 / G-01: per-role secrets mixed into the libp2p and ML-KEM
    /// identity derivations, so neither identity is recomputable from
    /// the public `node_id`. Generated by the ceremony, one pair per role.
    libp2p_seed_salt_hex: String,
    kem_seed_salt_hex: String,
}
...
fn build_node_json(
    ...
    serde_json::json!({
        ...
        "devnet": {
            ...
            // G-01: transport identities are (node_id, secret salt), never
            // node_id alone. See the `Libp2pSection` docs.
            "libp2p_seed_salt_hex": libp2p.libp2p_seed_salt_hex,
            "kem_seed_salt_hex": libp2p.kem_seed_salt_hex,
...
pub fn generate_ceremony_values(
    cfg: &CeremonyConfig,
) -> Result<(CeremonyValues, Vec<ValidatorEntry>)> {
    if cfg.validators == 0 {
    ...
    let libp2p_for = |role: NodeRole| -> Libp2pSection {
        ...
        let (libp2p_salt, kem_salt) = salts_for(role);
        Libp2pSection {
            listen_field: role.libp2p_listen_field(),
            listen_addr: "0.0.0.0:26656".to_string(),
            bootstrap_peers,
            libp2p_seed_salt_hex: hex::encode(libp2p_salt),
            kem_seed_salt_hex: hex::encode(kem_salt),
        }
    };
