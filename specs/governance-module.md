...
> **Reserved — `token_economics` (parts of this spec).** The public chain `viper-testnet-2` has no native token. The proposal lifecycle, proposal types, epoch-boundary execution, API and error codes in this document are active. The token-dependent parts — §1.4 stake-weighted voting power (with the feature off every active validator has a constant weight of 1), §3.1 `GovernanceDeposit` and §8 deposit mechanics including the deposit burn, and §5.5 `SlashParamUpdate` (its parameters have no effect while slashing dispatch is compiled out) — are implemented behind the `token_economics` Cargo feature, compiled out of the public chain build, and kept as a design reserve. Nothing in this document is an offer, a sale or a promise of any token or other asset.
