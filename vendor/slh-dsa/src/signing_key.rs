...
use ::signature::{Error, KeypairRef, Signer};
...
// RandomizedSigner impl removed: signature 2.3.0-pre.7+ changed the RandomizedSigner
// trait signature (now requires generic R: TryCryptoRng instead of impl CryptoRngCore).
// We use try_sign_with_context(msg, b"", None) for deterministic signing instead.
